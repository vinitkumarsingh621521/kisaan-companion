...
const Footer = forwardRef<HTMLElement>((_, ref) => {
  ...
  const copyLink = async () => {
    try { await navigator.clipboard.writeText(SHARE_URL); toast.success(t("footer.copied", "Link copied!")); }
    catch { toast.error("Copy failed"); }
  };

  // current season pill
  const m = new Date().getMonth() + 1;
  const season = m >= 6 && m <= 10
    ? t("footer.season.kharif", "🌧️ Kharif Season")
    : (m >= 11 || m <= 3)
      ? t("footer.season.rabi", "🌾 Rabi Season")
      : t("footer.season.zaid", "☀️ Zaid Season");
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-10">
            ...
            <div>
              ...
              <div className="space-y-2 text-sm text-background/60">
                <Link to="/crop-advisor" className="block hover:text-background transition-colors">{t("nav.cropAdvisor")}</Link>
                <Link to="/market" className="block hover:text-background transition-colors">{t("nav.market")}</Link>
                <Link to="/schemes" className="block hover:text-background transition-colors">{t("nav.schemes")}</Link>
                <Link to="/community" className="block hover:text-background transition-colors">{t("nav.community")}</Link>
              </div>
            </div>
            <div>
              <h4 className="font-display font-semibold mb-3">{t("footer.tools", "Tools")}</h4>
              <div className="space-y-2 text-sm text-background/60">
                <Link to="/tools/field-mapper" className="block hover:text-background transition-colors">{t("nav.fieldMapper")}</Link>
                <Link to="/tools/reports" className="block hover:text-background transition-colors">{t("nav.reports")}</Link>
                <Link to="/tools/satellite" className="block hover:text-background transition-colors">{t("nav.satellite")}</Link>
                <Link to="/tools/iot" className="block hover:text-background transition-colors">{t("nav.iot")}</Link>
                <Link to="/tools/achievements" className="block hover:text-background transition-colors">{t("nav.achievements")}</Link>
              </div>
            </div>
            <div>
              <h4 className="font-display font-semibold mb-3">{t("footer.contact", "Contact")}</h4>
              ...
              <h4 className="font-display font-semibold mb-2 text-sm">{t("footer.share", "Share")}</h4>
              <div className="flex items-center gap-2">
                ...
                <button onClick={copyLink} className="text-xs text-background/60 hover:text-background ml-2">{t("footer.copyLink", "Copy link")}</button>
              </div>
            </div>
          </div>
